...
fn main() -> ! {
    ...
    uart.write(b"\r\nRF1_IMAGE_OK\r\n");
    run_wifi_smoke(&uart, efuse, p.KM, p.SPACC, p.PKE, p.TRNG);
...
fn run_wifi_smoke(
    ...
    _pke: hisi_hal::peripherals::Pke<'_>,
...
fn run_wifi_smoke(
    ...
    let radio = match hisi_rf::init(
        ...
        ws63_rf_rs::hisi_rf_backend::resources(efuse, km, spacc, pke, trng),
        &RADIO_STATE,
    ) {
        Ok(radio) => radio,
        Err(error) => {
...
fn write_upstream_supplicant_diagnostics(uart: &Uart<'_, hisi_hal::peripherals::Uart0<'_>>) {
    ...
    let [
        ...
        add_total_ms,
        add_max_ms,
    ] = ws63_rf_rs::hardware_p256_diagnostic_snapshot();
    uart.write(b"RFDBG_HW_P256 requests=0x");
    uart.write(&hex8(requests));
    uart.write(b" failures=0x");
    uart.write(&hex8(failures));
    uart.write(b" total_ms=0x");
    ...
    uart.write(b" max_ms=0x");
    uart.write(&hex8(max_ms));
    uart.write(b" add_requests=0x");
    uart.write(&hex8(add_requests));
    uart.write(b" add_failures=0x");
    uart.write(&hex8(add_failures));
    uart.write(b" add_total_ms=0x");
    uart.write(&hex8(add_total_ms));
    uart.write(b" add_max_ms=0x");
    uart.write(&hex8(add_max_ms));
    uart.write(b"\r\n");
    let [
        requests,
        failures,
